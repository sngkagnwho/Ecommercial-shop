    ...
    public class AdminPaymentService : IAdminPaymentService
    {
        ...
        private void SetAuthHeader(HttpClient httpClient)
        {
            ...
            if (!string.IsNullOrEmpty(token))
            {
                ...
                _logger.LogInformation($"✅ JWT token set successfully. Token length: {token.Length}");
            }
            else
            {
                _logger.LogWarning("⚠️ JWT token is NULL or EMPTY - User may not be authenticated!");
            }
        }
        ...
        public async Task<List<PaymentMethodViewModel>> GetPaymentMethodsAsync()
        {
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    throw new InvalidOperationException(BuildBackendErrorMessage("Kh�ng th? t?i danh s�ch ph??ng th?c thanh to�n", errorContent));
                }
        ...
        public async Task<PaymentMethodViewModel> GetPaymentMethodByCodeAsync(string code)
        {
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    var errorContent = await response.Content.ReadAsStringAsync();
                    throw new InvalidOperationException(BuildBackendErrorMessage("Kh�ng t�m th?y ph??ng th?c thanh to�n", errorContent));
                }
        ...
        public async Task<bool> DeletePaymentMethodAsync(int id)
        {
            try
            {
                ...
                var errorContent = await response.Content.ReadAsStringAsync();
                throw new InvalidOperationException(BuildBackendErrorMessage("X�a ph??ng th?c thanh to�n th?t b?i", errorContent));
            }
            catch (Exception ex)
            {
